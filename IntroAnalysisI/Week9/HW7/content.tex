...
\begin{proof}
    Observe that for all fixed $x \in [0, 1)$, $g(x)$ is constant and $\lim_{n \to \infty} x^{n} = 0$, so $\lim_{n \to \infty} g(x)x^{n} = 0$, and when $x=1, g(1) = 0$, so $\lim_{n \to \infty} g(x)x^{n} = 0$, and hence $g_n(x)x^n$ converge to $0$ on $[0,1]$ pointwisely. \\
    Then we show that $g_n(x)x^n$ converge uniformly to 0. \\
    By extreme value theorem, since $[0,1]$ is closed set on $\mathbb{R}$ and $g$ is continuous function, so there exist $M$ such that $\lvert g(x) \rvert \le M, \forall x \in [0,1]$. \\
    For fixed $\varepsilon > 0$, we separate to two cases.
    \begin{itemize}
        \item Near $x = 1$: Since $g$ is continuous, $\exists \delta > 0$ such that $\lvert x-1 \rvert < \delta \implies \lvert g(x) - 0 \rvert < \varepsilon$. And since $\lvert x^n \rvert \le 1, \forall x \in [0,1]$, this imply that 
        \[
            \lvert x-1 \rvert < \delta \implies \lvert g(x) x^n \rvert < \varepsilon
        \]
        Thus, for $x \in (1-\delta, 1]$, $n \in \mathbb{N}$, $\lvert g(x) x^n \rvert < \varepsilon$.
        \item Away from $x = 1$: for $x \in [0, 1-\delta]$, we know $\lvert x^n \rvert \le (1-\delta)^n$, as $n \to \infty$, $(1-\delta)^n \to 0$, so $\exists N$ such that 
        \[ 
        \forall n \ge N, (1-\delta)^n < \frac{\varepsilon}{M}.
        \]
        So 
        \[ 
        \forall n \ge N, \lvert g(x) x^n \rvert \le M(1-\delta)^n < M\cdot \frac{\varepsilon}{M} = \varepsilon. 
        \]
    \end{itemize}
    And combine two cases above, by definition $\{g(x)x^n\}$ is indeed uniformly converge to 0 on $[0,1]$.
\end{proof}
%────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
\begin{problem}[15pts]
...
\begin{proof}
    \vphantom{text}
    \begin{itemize}
        \item The limit function $f$ of the sequence $\{ f_n \}$: \\
        Fix $x \in \mathbb{R}$. If $x = 0, f_n(x) = 0$. If $x \neq 0$, then $nx^2 \to \infty$ when $n \to \infty$, so 
        \[
        \lim_{n \to \infty}f_n(x) = \frac{x}{1+nx^2} = 0.
        \]
        And hence the pointwise limit is $f(x) = 0$.
        \item The limit function $g$ of the sequence $\{ f_n' \}$: \\
        \[
        f_n(x) = x(1 + nx^2)^{-1}, f'_n(x) = (1 + nx^2)^{-1} - x(1 + nx^2)^{-2}(2nx)
        \]
        So
        \[
        f'_n(x) = \tfrac{1-nx^2}{(1+nx^2)^2}
        \]
        Fix $x \in \mathbb{R}$. If $x = 0, f'_n(x) = 1$. If $x \neq 0$, let $t = nx^2$,
        \[
        \lim_{n \to \infty} \tfrac{1-nx^2}{(1+nx^2)^2} = \lim_{t \to \infty} \tfrac{1-t}{(1+t)^2} \overset{\text{L'H}}{=} \lim_{t \to \infty} \tfrac{-1}{2(1+t)} = 0
        \]
        So the pointwise limit $g$ is 
        \[
        g(x) =
        \begin{cases}
        1, & x = 0, \\
...
\begin{proof}[(a)]
    Since $f$ is constant function, $f'$ exist and $f'(x) = 0$.\\
    Since $g(0) = 1$, $f'(0) \neq g(0)$. And $\forall x \in \mathbb{R} \setminus \{0\}, f'(x) = g(x) = 0$.
\end{proof}

\begin{proof}[(b)]
    First, we show that $f_n(x)$ have global maximum. \\
    Let $\phi_n(x) = \tfrac{x}{1+nx^2}, x \ge 0$, $\phi'_n(x) = \tfrac{1-nx^2}{(1+nx^2)^2} = 0 \implies x = \tfrac{1}{\sqrt{n}}$. \\
    At $x = \tfrac{1}{\sqrt{n}}$, $\phi_n(x) = \tfrac{1}{2\sqrt{n}}$. Since $\lvert f_n \rvert$ is even function, $\tfrac{1}{2\sqrt{n}}$ is also the global maximum of $\lvert f_n \rvert$ on $\mathbb{R}$.\\
    Notice that 
    \[
    \forall x \in \mathbb{R}, \lim_{n \to \infty} \lvert f_n(x) \rvert \le \lim_{n \to \infty} \tfrac{1}{2\sqrt{n}} = 0.
    \]
    So $f_n$ is indeed converge uniformly to 0 on $\mathbb{R}$. 
\end{proof}

\begin{proof}[(c)]
    For any interval not contain origin, take any interval $[a, b]$ with $0 < a < b$. The for all $x \in [a,b]$,
    \[
    \lvert f'_n(x) \rvert = \tfrac{\lvert 1-nx^2 \rvert}{(1+nx^2)^2} \le \tfrac{1+nx^2}{(1+nx^2)^2} = \tfrac{1}{(1+nx^2)} \le \tfrac{1}{(1+na^2)}
    \]
    , and we know $\lim_{n \to \infty} \tfrac{1}{(1+na^2)} = 0$, and $g(x) = 0$ for all $x \ne 0$, so
    \[
    \sup_{x \in [a,b]} \lvert f'_n(x) - g(x) \rvert = \sup_{x \in [a,b]} \lvert f'_n(x) \rvert \le \tfrac{1}{(1+na^2)} \to 0
    \]
    , so $f'_n$ converge uniformly on $[a,b]$. \\
    And we can do the same argument for $[-b, -a]$ with $0 < a < b$, and so as $(-\infty, c]$ or $[c, \infty)$ with $c > 0$ since the same bound $\tfrac{1}{(1+nc^2)}$ works. \\
    So $f'_n$ converges uniformly to $g$ on any interval not contain the origin. \\
    Then we show that if the interval containing the origin, $f'_n$ is not uniformly converge to $g$. \\
    % Suppose it is, then $\exists a > 0$, $\{f'_n\}$ converge uniformly on $[-a, a]$. \\
    Notice that 
    \[
    \lvert f'_n(x) \rvert = \lvert \tfrac{1-nx^2}{(1+nx^2)^2} \rvert
    \]
    if we take $x_n = \tfrac{1}{2\sqrt{n}}$,
    \[
    \lvert f'_n(x_n) \rvert = \lvert \tfrac{\tfrac{3}{4}}{(\tfrac{5}{4})^2} \rvert = \frac{12}{25}, \text{which is a constant}
    \]
    And we know when $n \ge \tfrac{1}{4a^2}$, $x_n \in [-a, a]$.\\

    Then we know that $\exists\varepsilon = \tfrac{1}{48763 } < \frac{12}{25}$, $\forall N \in \mathbb{N}$, there exists $n' = \max\{\tfrac{1}{4a^2}, N\} \ge N$ such that $x \in [-a,a] \setminus \{0\}$, $\lvert f_{n'}(x_{n'}) - 0 \rvert > \varepsilon$, and hence $f'_n(x)$ not uniformly converge to $g$ on any interval containing the origin. 
...
\begin{proof}
    First, notice that $n^2x^2 \ge 0$, so when $x = 0$, $e^{-n^2x^2}$ have global maximum value 1, and furthermore, it is obvious that $\forall x \in \mathbb{R}$, $f_n(x) \ge 0$. This means $\sup_{x \in \mathbb{R}} \lvert f_n(x) \rvert = \tfrac{1}{n}$. \\
    And 
    \[
    \forall x \in \mathbb{R}, \lim_{n \to \infty}\lvert f_n(x) \rvert \le  \lim_{n \to \infty} \sup_{x \in \mathbb{R}} \lvert f_n(x) \rvert = \lim_{n \to \infty} \tfrac{1}{n} = 0
    \]
    So $f_n$ converge uniformly to 0 on $\mathbb{R}$.

    \[
    f'_n(x) = \tfrac{1}{n}e^{-n^2x^2}(-2n^2x) = -2nxe^{-n^2x^2}
    \]
    Fixed $x \in \mathbb{R}$, when $x = 0$, $f'_n(x) = -2n \cdot 0 \cdot e^{-n^2 \cdot 0} = 0$, and when $x \ne 0$,
    \[
    \lim_{n \to \infty} f'_n(x) = (-2x) \cdot \lim_{n \to \infty} \exp(-n^2x^2 + \ln(n)) = -2x \cdot 0 = 0.
    \]
    So $f'_n$ converge pointwisely to 0 on $\mathbb{R}$.

    \begin{claim}
        $f'_n$ is not uniformly converge on any interval containing the origin.
    \end{claim}
    \begin{explanation}
        % Suppose it is, then $\exists a > 0$, $\{f'_n\}$ converge uniformly on $[-a, a]$. \\
        Notice that 
        \[
        \lvert f'_n(x) \rvert =  -2n \lvert x \rvert e^{-n^2x^2}
        \]
        if we take $x_n = \tfrac{1}{\sqrt{2}n}$,
        \[
        \lvert f'_n(x_n) \rvert = \sqrt{2}e^{-\frac{1}{2}}, \text{which is a constant}
        \]
        And we know when $n \ge \tfrac{1}{\sqrt{2}a} = N$, $x_n \in [-a, a]$.\\

        Then we know that $\exists\varepsilon = \tfrac{1}{48763 } < \sqrt{2}e^{-\frac{1}{2}}$, $\forall N \in \mathbb{N}$, there exists $n' = \max\{\tfrac{1}{\sqrt{2}a}, N\} \ge N$ such that $x \in [-a,a]$, $\lvert f_{n'}(x_{n'}) - 0 \rvert > \varepsilon$, and hence $f'_n(x)$ not uniformly converge to $0$ on any interval containing the origin. 
        % So $\forall n \ge N$, $\exists x_n \in [-a,a]$ such that $\lvert f'_n(x_n) - 0\rvert > \tfrac{1}{48763}$, which is a contradiction, and hence $f'_n(x)$ not uniformly converge to $0$ on any interval containing the origin.
    \end{explanation}
\end{proof}
%────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
\begin{problem}[10pts]
